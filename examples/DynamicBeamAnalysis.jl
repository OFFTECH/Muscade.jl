...
file_path = "C:\\Users\\marekj\\Downloads\\Muscade.jl\\examples\\SCR.csv"
df = CSV.read(file_path, DataFrame; delim = ' ')
dynMotionX = vcat(
    0,
    df[:, "Displacement in x - direction"] .- df[1, "Displacement in x - direction"],
    df[end, "Displacement in x - direction"] - df[1, "Displacement in x - direction"],
)
dynMotionZ = vcat(
    0,
    df[:, "Displacement in z - direction"] .- df[1, "Displacement in z - direction"],
    df[end, "Displacement in z - direction"] - df[1, "Displacement in z - direction"],
)
dynMotionT = vcat(-10.0, df[:, "x"], df[end, "x"] + 100)
...
x1_ρₛ = 7850.0                     # Steel denstiy [kg/m^3]
x1_ρᵢ = 200.0                      # Internal fluid density [kg/m^3]
...
x2_ρₛ = 7850.0                     # Steel denstiy [kg/m^3]
x2_ρᵢ = 200.0                      # Internal fluid density [kg/m^3]
...
segLength = [300.0, 300.0, 80.0] # Segment lengths
...
nodeCoord = [
    hcat(
        accLength[seg] .+ ((1:nnodes[seg]) .- 1) / (nnodes[seg] - 1) * segLength[seg],
        0 .+ zeros(Float64, nnodes[seg], 1),
        -300 .+ zeros(Float64, nnodes[seg], 1),
    ) for seg = 1:nseg
];
...
elementList  = addelement!(model, EulerBeam3D, mesh; mat = xSection[1], orient2 = SVector(0.0, 1.0, 0.0))
firstNode[1] = nodid[1]
...
if nseg >= 2
    for segid ∈ 2:nseg
        ...
        global elementList = vcat(
            elementList,
            addelement!(
                model,
                EulerBeam3D,
                [firstNode[segid], nodid[1]];
                mat = xSection[segid],
                orient2 = SVector(0.0, 1.0, 0.0),
            ),
        )
        global elementList = vcat(
            elementList,
...
@functor with(x1_w, segLength, nel) weight1(t) =
    -((min(t, -5.0) + 10) / 5) * x1_w * segLength[1] / nel[1];
@functor with(x2_w, segLength, nel) weight2(t) =
    -((min(t, -5.0) + 10) / 5) * x2_w * segLength[2] / nel[2];
@functor with(x1_w, segLength, nel) weight3(t) =
    -((min(t, -5.0) + 10) / 5) * x1_w * segLength[3] / nel[3];
...
staticLoadSteps = (-10:0.2:0) * 1.0
nStaticLoadSteps = length(staticLoadSteps)
staticStates = solve(
    SweepX{0};
    initialstate,
    time = staticLoadSteps,
    verbose = true,
    maxΔx = 1e-6,
    maxiter = 100,
);
